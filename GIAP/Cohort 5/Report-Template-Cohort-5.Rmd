...
knitr::include_graphics("pics/NICHE Practice Model.png")
```
...
knitr::include_graphics("pics/GIAP Practice Model.png")
```
...
knitr::include_graphics("pics/NICHE Action Plan.png")
```
...
```{r action_plan_table}
htmltools::includeHTML("tables/action_plan_table_code.txt")
```
...
htmltools::includeHTML("tables/recommendations_table_code.txt")
```
...
A total of `r response[response$Hospital.Name==name,"Completions"]` nurses at `r name` provided compete data for the questions included in this report. Of the eligible staff invited to participate in the GIAP, `r round(response[response$Hospital.Name==name,"Rate"]*100,2)`% completed the survey. As you review the your hospitals’ results, keep in mind that a mean score of 80% on each of the GIAP sections demonstrates nurses’ mastery of geriatric nursing concepts and reflects a unit environment that supports age-friendly care.
