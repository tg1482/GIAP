---
title: "Winter 2017 Cohort"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE, comment=NA, message=FALSE, warning=FALSE, fig.align = 'center', fig.width = 6, fig.asp=0.62, out.width = "70%")
options(width=190)
```
...
DF <- read_csv("~/Desktop/NICHE/Implementation/Winter 2017/NICHE Program Year 1 Implementation Survey - Winter 2017 Cohort.csv")

DF <- DF[-c(1,2),]

DF <- to_factor(DF)

impdim <- read.xlsx("~/Desktop/NICHE/Implementation/Implementation_Dimensions.xlsx", sheet = "Winter 2017 Cohort")
...
values <- c()
for(i in 1:nrow(DF)){
  row_values <- strsplit(DF$Q10[i], split=',(?!\\s)', perl = T)
  values <- append(values, row_values)
}

values <- unlist(values)
values <- values[values != "Other. Please provide response here:"]
...
dat <- dat %>% 
  mutate(Var1 = factor(Var1, levels = rev(c("90-100%", "75-90%", "45-75%", "20-45%", "Less than 20%"))))
...
dat <- dat %>% 
  mutate(Var1 = factor(Var1, levels = rev(c("25-40 hours", "13-24 hours", "6-12 hours", "2-5 hours", 
                                            "Less than 2 hours"))))
...
values <- c()
for(i in 1:nrow(DF)){
  row_values <- strsplit(DF$Q20[i], split=',(?!\\s)', perl = T)
  values <- append(values, row_values)
}
...
temp <- DF %>% 
  select(Q24) %>% 
  mutate(Q24 = factor(Q24, levels = c("One month", "Two months", "Three months", "Four months", "Five months",
                                      "Six months", "Seven months", "Eight months", "Nine months", "Ten months",
                                      "Eleven months", "Twelve months", "More than one year"))) 
table(temp$Q24) %>%
  as.data.frame() %>%
...
```{r}
ggplot(DF, aes(x=as.numeric(Q26))) + geom_histogram(fill=NICHE_colors[1], color="white") + 
...
values <- c()
for(i in 1:nrow(DF)){
  row_values <- strsplit(DF$Q28[i], split=',(?!\\s)', perl = T)
  values <- append(values, row_values)
}
...
values <- c()
for(i in 1:nrow(DF)){
  row_values <- strsplit(DF$Q31[i], split=',(?!\\s)', perl = T)
  values <- append(values, row_values)
}
